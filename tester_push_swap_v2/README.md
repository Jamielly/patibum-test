...
Script de testes automatizados para o projeto **push_swap**, construído com
base no projeto. Ele roda a maior parte dos itens verificáveis da defesa
(exceto os que dependem de interação humana, como explicação oral do
código) e no final entrega um resumo de aprovação/reprovação por seção,
...
- **Economizar tempo na defesa**: em vez de digitar cada comando na mão, você
  roda um único comando e já recebe o resultado de dezenas de casos.
...
- **Eliminar dependência de binário externo**: o checker é compilado na
  hora, na sua máquina, então não há risco de permissão perdida, arquitetura
  incompatível (x86/ARM) ou binário ausente.
...
Gera `./checker_linux`, compilado com `-Wall -Wextra -Werror`. Não precisa
baixar nem copiar nenhum binário do campus.
...
`REPO`, o comando assume o diretório atual:

```bash
make test
```

`make test` compila o checker automaticamente se ele ainda não existir —
não precisa rodar `make checker` antes, é só conveniência ter os dois
comandos separados.

### 3. Rodar o script diretamente (sem make)

Também funciona chamando o `.sh` na mão, se preferir:
...
Por padrão o script assume `./push_swap` e tenta detectar automaticamente
`checker_linux`, `checker_Mac` ou `fedora_checker` na pasta testada — e
agora aplica `chmod +x` sozinho se encontrar o checker sem permissão de
execução.
...
Se o binário `push_swap` não compilar, as seções que dependem dele aparecem
como **puladas**, não escondidas — o script sempre chega até o resumo
final. O mesmo vale se o checker não for encontrado: as seções de
corretude ficam marcadas como puladas, com um aviso separado no resumo
(isso não é mais tratado como falha de compilação).

## 📦 Requisitos

- **Bash** (Linux ou macOS)
- **`cc`/`gcc`/`clang`** — para compilar o `checker.c` (qualquer compilador C
  padrão serve)
...
- O checker próprio (`checker.c`) segue a régua padrão de erro do push_swap
  (número não numérico, duplicado, overflow → `Error` no stderr); se o seu
  campus usa uma variação diferente da régua, ajuste `checker.c` antes de
  compilar.
...
- Nome/flags do compilador do checker → variáveis `CC` e `CFLAGS` no topo do
  `Makefile`
